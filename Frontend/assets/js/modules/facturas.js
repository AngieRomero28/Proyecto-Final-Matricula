...
window.Modules.facturas = (function () {
    ...
    function aplicarFiltros() {
        const texto = String(document.getElementById('filtro-factura-buscar')?.value || '').trim().toLowerCase();
        const estado = String(document.getElementById('filtro-factura-estado')?.value || '').trim();
    ...
    function renderResumen() {
        ...
        const totalMonto = facturasFiltradas.reduce((acc, item) => acc + Number(item.Total || 0), 0);
    ...
    function renderTabla() {
        ...
        tabla.innerHTML = facturasFiltradas.map((factura) => `
            <tr>
                ...
                <td>${escapeHtml(factura.NombreEstudiante || '')}</td>
                <td>${escapeHtml(factura.NombrePeriodo || '')}</td>
                <td>${Helpers.formatCurrency(factura.Total || 0)}</td>
                <td>${Helpers.formatCurrency(factura.MontoPagado || 0)}</td>
                <td>${Helpers.formatCurrency(factura.SaldoPendiente || 0)}</td>
                <td>
                    <span class="badge ${getBadgeEstado(factura.EstadoFactura)}">
                        ${escapeHtml(factura.EstadoFactura || 'N/D')}
                    </span>
                </td>
                <td>
                    <button class="btn btn-outline" onclick="Modules.facturas.verDetalle(${factura.FacturaID})">
                        Ver
                    </button>
                </td>
            </tr>
            ...
            body: `
                <div class="resumen-financiero">
                    ...
                    <div class="mini-card">
                        ...
                        <div class="mini-card-value">${Helpers.formatCurrency(factura.Total || 0)}</div>
                    </div>
                    <div class="mini-card">
                        <div class="mini-card-label">Saldo</div>
                        <div class="mini-card-value">${Helpers.formatCurrency(factura.SaldoPendiente || 0)}</div>
                    </div>
                </div>

                <div class="mt-2">
                    <p><strong>Estudiante:</strong> ${escapeHtml(factura.NombreEstudiante || '')}</p>
                    ...
                    <p><strong>Subtotal:</strong> ${Helpers.formatCurrency(factura.Subtotal || 0)}</p>
                    <p><strong>Descuento:</strong> ${Helpers.formatCurrency(factura.Descuento || 0)}</p>
                    <p><strong>Total:</strong> ${Helpers.formatCurrency(factura.Total || 0)}</p>
                    <p><strong>Monto pagado:</strong> ${Helpers.formatCurrency(factura.MontoPagado || 0)}</p>
                    <p><strong>Saldo pendiente:</strong> ${Helpers.formatCurrency(factura.SaldoPendiente || 0)}</p>
                    <p><strong>Estado factura:</strong> ${escapeHtml(factura.EstadoFactura || '')}</p>
