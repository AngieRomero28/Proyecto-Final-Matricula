...
window.Modules.adminCursos = (function () {
    let cursos = [];
    ...
    function configurarEventos() {
        const btnNuevo = document.getElementById('btn-admin-nuevo-curso');
    ...
    async function manejarRecargaCursos() {
        window.UI.clearMessage('admin-cursos-message');

        try {
            await cargarCursos();
            window.UI.showMessage(
                'admin-cursos-message',
                'success',
    ...
    async function cargarCursos() {
        const tabla = document.getElementById('tabla-admin-cursos');
        if (!tabla) return;
        ...
        tabla.innerHTML = cursos.map((cur) => {
            ...
            return `
                <tr>
                    ...
                    <td>
                        <button class="btn btn-outline" onclick="window.Modules.adminCursos.ver(${Number(cur.CursoID)})">
                            Ver
                        </button>
                    </td>
                </tr>
